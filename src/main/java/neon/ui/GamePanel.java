...
/**
 * This class represents the main game screen. It contains a <code>JVectorPane</code> overlayed with
 * player information, and a text area to show in-game messages.
 *
 * @author mdriesen
 */
...
public class GamePanel extends JComponent {
  ...
  public void repaint() {
    ...
    Collection<Renderable> renderables =
        Engine.getAtlas().getCurrentZone().getRenderables(getVisibleRectangle());
  ...
  private void drawStats() {
    ...
    if (stats.getCon() > (int) player.species.con) {
      conLabel.setText(
          "<html>constitution: <font color=green>" + stats.getCon() + "</font></html>");
    ...
    if (stats.getInt() > (int) player.species.iq) {
      intLabel.setText(
          "<html>intelligence: <font color=green>" + stats.getInt() + "</font></html>");
    ...
    if (player.getConditions().contains(Condition.DISEASED)
        || player.getConditions().contains(Condition.CURSED)
    ...
    public BufferedImage filter(BufferedImage src, BufferedImage dest) {
      ...
      for (Point p : Engine.getAtlas().getCurrentZone().getLightMap().keySet()) {
        ...
        area.subtract(
            new Area(
                new Ellipse2D.Float(
                    (p.x - 4) * zoom - view.x * zoom,
                    (p.y - 4) * zoom - view.y * zoom,
                    9 * zoom,
                    9 * zoom)));
      }

      if (image == null
          || image.getWidth() != src.getWidth()
          || image.getHeight() != src.getHeight()) {
