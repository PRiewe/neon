...
public class TravelDialog implements KeyListener {
  ...
  public TravelDialog(UserInterface ui, MBassador<EventObject> bus, GameStores gameStores) {
    this.bus = bus;
    this.ui = ui;
    ...
    JLabel instructions =
        new JLabel("Use arrow keys to select destination, press enter to travel, esc to exit.");
  ...
  public void keyPressed(KeyEvent e) {
    switch (e.getKeyCode()) {
      ...
      case KeyEvent.VK_ENTER:
        try {
          if (player.getInventoryComponent().getMoney()
              >= costData.get(destinations.getSelectedValue())) {
  ...
  private void initDestinations() {
    ...
    for (RPerson.Service service :
        ((RPerson) gameStores.getResources().getResource(agent.getID())).services) {
