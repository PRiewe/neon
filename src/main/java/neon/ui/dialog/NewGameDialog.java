  ...
  private class Ok extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      if (name.getText().equals("")) {
        ...
        ui.showMessage(
            "There is already a character with the given name. <br>"
                + "Choose another name or remove the existing character.",
            3);
        name.requestFocus();
      } else {
        bus.publishAsync(
            new LoadEvent(
                this,
                raceList.get(race.getSelectedItem()),
                name.getText(),
                (Gender) gender.getSelectedItem(),
                (Player.Specialisation) spec.getSelectedItem(),
                prof.getText(),
