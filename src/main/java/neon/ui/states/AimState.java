...
/**
 * Implements all methods to aim at something. This is done by showing a cursor on the main game
 * field.
 *
 * @author mdriesen
 */
public class AimState extends State implements KeyListener {
  ...
  public void enter(TransitionEvent e) {
    ...
    panel.print(
        "Use arrow keys to move cursor. Press L to cancel, "
  ...
  private void talk() {
    ...
    if (target.distance(bounds.getLocation()) < 2) {
      ...
      if (creature != null) {
        ...
        bus.publishAsync(
            new TransitionEvent("return", "message", "No person to talk to selected."));
      }
    } else {
