...
public class InventoryState extends State implements KeyListener, MouseListener {
  ...
  public InventoryState(
    ...
    magicHandler = new MagicHandler(context.getQueue(), gameStores, context);
    inventoryHandler = new InventoryHandler(gameStores.getStore());
    // info
    info = new JLabel();
    info.setBorder(
        new CompoundBorder(new TitledBorder("Information"), new EmptyBorder(0, 5, 10, 5)));
    ...
    JLabel instructions =
        new JLabel(
            "<html>Use arrows to select item, press enter to use/eat/(un)equip item, "
