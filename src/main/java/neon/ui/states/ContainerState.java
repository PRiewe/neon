...
public class ContainerState extends State implements KeyListener, ListSelectionListener {
  ...
  public ContainerState(
    ...
    JLabel instructions =
        new JLabel(
            "Use arrows keys to select item, press space to pick up/drop item, ctrl to exit.");
    ...
    switch (key.getKeyCode()) {
      ...
      case KeyEvent.VK_RIGHT:
        if (iList.hasFocus()) {
          ...
          cScroll.setBorder(
              new TitledBorder(new LineBorder(line.brighter(), 2), container.toString()));
  ...
  public void valueChanged(ListSelectionEvent lse) {
    Entity item =
        iList.getSelectedValue() != null ? iList.getSelectedValue() : cList.getSelectedValue();
