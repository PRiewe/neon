...
public class SkillHandler {
  public static int check(Creature creature, Skill skill) {
    ...
    switch (skill) { // bonussen
      case ALTERATION:
        ...
        break;
      case AXE:
        if (characteristics.hasTrait(Trait.WEAPON_FINESSE_AXE)) {
          check += 2;
        }
        break;
      case BLADE:
        if (characteristics.hasTrait(Trait.WEAPON_FINESSE_BLADE)) {
          check += 2;
        }
        break;
      case CLIMBING:
        if (characteristics.hasTrait(Trait.AGILE)) {
          check += 2;
        }
        if (characteristics.hasTrait(Trait.ENDURANCE)) {
          check += 1;
        }
        break;
      case CREATION:
        if (characteristics.hasTrait(Trait.MAGICAL_APTITUDE_CREATION)) {
          check += 2;
        }
        break;
      case DESTRUCTION:
        if (characteristics.hasTrait(Trait.MAGICAL_APTITUDE_DESTRUCTION)) {
          check += 2;
        }
        break;
      case ILLUSION:
        if (characteristics.hasTrait(Trait.MAGICAL_APTITUDE_ILLUSION)) {
          check += 2;
        }
        break;
      case LOCKPICKING:
        if (characteristics.hasTrait(Trait.NIMBLE_FINGERS)) {
          check += 2;
        }
        break;
      case MERCANTILE:
        if (characteristics.hasTrait(Trait.PERSUASIVE)) {
          check += 2;
        }
        break;
      case SWIMMING:
        if (characteristics.hasTrait(Trait.ATHLETIC)) {
          check += 2;
        }
        if (characteristics.hasTrait(Trait.ENDURANCE)) {
          check += 1;
        ...
        break;
      case RESTORATION:
        if (characteristics.hasTrait(Trait.MAGICAL_APTITUDE_RESTORATION)) {
          check += 2;
        }
        break;
      case DODGING:
        if (characteristics.hasTrait(Trait.ACROBATIC)) {
          check += 2;
        }
        if (characteristics.hasTrait(Trait.LIGHTNING_REFLEXES)) {
          check += 1;
        }
        break;
      case BLOCK:
        ...
        break;
      case SNEAK:
        if (characteristics.hasTrait(Trait.STEALTHY)) {
          check += 2;
        }
        break;
      case LIGHT_ARMOR:
        if (characteristics.hasTrait(Trait.ARMOR_PROFICIENCY_LIGHT)) {
          check += 2;
        }
        break;
      case MEDIUM_ARMOR:
        if (characteristics.hasTrait(Trait.ARMOR_PROFICIENCY_MEDIUM)) {
          check += 2;
        }
        break;
      case HEAVY_ARMOR:
        if (characteristics.hasTrait(Trait.ARMOR_PROFICIENCY_HEAVY)) {
          check += 2;
        }
        break;
      case DISGUISE:
        if (characteristics.hasTrait(Trait.DECEITFUL)) {
          check += 2;
        }
        break;
      case SPEAR:
        if (characteristics.hasTrait(Trait.WEAPON_FINESSE_SPEAR)) {
          check += 2;
        }
        break;
      case BLUNT:
        if (characteristics.hasTrait(Trait.WEAPON_FINESSE_BLUNT)) {
          check += 2;
        }
        break;
      case ARCHERY:
        if (characteristics.hasTrait(Trait.WEAPON_FINESSE_ARCHERY)) {
          check += 2;
        }
        break;
      case UNARMED:
        if (characteristics.hasTrait(Trait.WEAPON_FINESSE_UNARMED)) {
          check += 2;
        }
        break;
      default:
  ...
  private static int getStatValue(Skill skill, Creature creature) {
    switch (skill.stat) {
      ...
      case CONSTITUTION:
        return creature.getStatsComponent().getCon();
      case DEXTERITY:
        return creature.getStatsComponent().getDex();
      case INTELLIGENCE:
        return creature.getStatsComponent().getInt();
      case WISDOM:
        return creature.getStatsComponent().getWis();
      case CHARISMA:
        return creature.getStatsComponent().getCha();
      default:
        return 0;
    }
  }
    ...
    if (value < player.getSkill(skill)) { // skill is met 1 gestegen
      ...
      switch (skill.stat) {
        ...
        case DEXTERITY:
          ...
          break;
        case INTELLIGENCE:
          player.addBaseInt(0.1f);
          break;
        case WISDOM:
          player.addBaseWis(0.1f);
          break;
        case CHARISMA:
          player.addBaseCha(0.1f);
          break;
        default:
          break;
      }
  ...
  public static void checkFeat(Skill skill, Player player) {
    ...
    switch (skill) {
      ...
      case ARMORER:
        ...
        if (player.getSkill(skill) >= 40
            && player.getSkill(Skill.ENCHANT) >= 40
      ...
      case ENCHANT:
        ...
        if (player.getSkill(skill) >= 20
            && player.getSkill(Skill.ARMORER) >= 20
            && !characteristics.hasFeat(Feat.CRAFT_MAGIC_ARMS_AND_ARMOR)) {
          characteristics.addFeat(Feat.CRAFT_MAGIC_ARMS_AND_ARMOR);
        }
        break;
      case RIDING:
        ...
        if (player.getSkill(skill) >= 40
            && player.getSkill(Skill.ARCHERY) >= 40
            && !characteristics.hasFeat(Feat.MOUNTED_ARCHERY)) {
          characteristics.addFeat(Feat.MOUNTED_ARCHERY);
        }
        break;
