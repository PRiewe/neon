...
/**
 * Renders items using JavaFX Canvas. Uses character-based textures from FXTextureFactory.
 *
 * @author mdriesen
 */
public class FXItemRenderComponent implements FXRenderable {
  protected final Item item;
  private int z = 0;

  public FXItemRenderComponent(Item item) {
    this.item = item;
  }
  ...
  public void paint(GraphicsContext gc, float zoom, boolean isSelected) {
    ...
    // Get glyph and color from item resource
    String glyph = "?"; // Default glyph
    String colorName = "white"; // Default color

    if (item.resource != null) {
      glyph = item.resource.text;
      colorName = item.resource.color;
    }
  ...
  /**
   * Get the item this component renders.
   *
   * @return the item
   */
  public Item getItem() {
