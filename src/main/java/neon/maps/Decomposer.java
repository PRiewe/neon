...
public class Decomposer {
  ...
  private static class Node {
    ...
    public void addPoint(int x, int y) {
      if (one == null) {
        Rectangle rOne =
            new Rectangle(bounds.x, bounds.y, Math.abs(x - bounds.x), Math.abs(y - bounds.y));
        one = new Node(rOne);
        Rectangle rTwo =
            new Rectangle(
                x, bounds.y, Math.abs(bounds.width - (x - bounds.x)), Math.abs(y - bounds.y));
        two = new Node(rTwo);
        Rectangle rThree =
            new Rectangle(
                bounds.x, y, Math.abs(x - bounds.x), Math.abs(bounds.height - (y - bounds.y)));
        three = new Node(rThree);
        Rectangle rFour =
            new Rectangle(
                x,
                y,
                Math.abs(bounds.width - (x - bounds.x)),
                Math.abs(bounds.height - (y - bounds.y)));
        four = new Node(rFour);
