...
public class WildernessTerrainGenerator {
  ...
  public WildernessTerrainGenerator() {
    this(new MapUtils(), new Dice());
  }

  public WildernessTerrainGenerator(MapUtils mapUtils, Dice dice) {
  ...
  public String[][] generateTerrainOnly(Rectangle r, RRegionTheme theme, String base) {
    // generate terrain
    String[][] terrain = generateTerrain(r.width, r.height, theme, base);
  ...
  String[][] generateTerrain(int width, int height, RRegionTheme theme, String base) {
    ...
    switch (theme.type) {
      case CHAOTIC:
        generateSwamp(width, height, theme, terrain);
        break;
      case PLAIN:
        generateForest(width, height, theme);
        break;
      case RIDGES:
        generateRidges(width, height, theme, terrain);
        break;
      case TERRACE:
        generateTerraces(width, height, theme, terrain);
        break;
      default:
        break;
    }
  ...
  private String[][] makeBorder(String type, String[][] terrain) {
  ...
  private String[][] addTerrain(
      int x, int y, int width, int height, String type, String[][] terrain) {
  ...
  protected String[][] generateSwamp(
      int width, int height, RRegionTheme theme, String[][] terrain) {
  ...
  void addVegetation(int width, int height, RRegionTheme theme, String base, String[][] terrain) {
    if (!theme.vegetation.isEmpty()) {
      ...
      for (int i = 0; i < fauna.length; i++) {
        for (int j = 0; j < fauna[i].length; j++) {
          ...
          // Eventually more dynamic.
          Set<String> swimTerrain = Set.of("lake", "sea", "lava");
          if (fauna[i][j] != null && !swimTerrain.contains(region)) {
