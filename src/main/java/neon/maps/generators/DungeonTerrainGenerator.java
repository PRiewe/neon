...
public class DungeonTerrainGenerator {
  ...
  public DungeonTerrainGenerator() {
    this(new MapUtils(), new Dice());
  }

  public DungeonTerrainGenerator(MapUtils mapUtils, Dice dice) {
  ...
  public record TilesAndTerrain(int[][] tiles, String[][] terrain) {}

  public TilesAndTerrain generateTilesOnly(RZoneTheme theme) {
    ...
    // features
    generateFeatures(theme.features, ratio, tiles, terrain);
    ...
    return new TilesAndTerrain(tiles, terrain);
  }
   ...
   */
  public int[][] generateBaseTiles(String type, int width, int height) {
    int[][] tiles = new int[width][height];
    switch (type) {
      case "cave" ->
          tiles = makeTiles(mazeGenerator.generateSquashedMaze(width, height, 3), width, height);
      case "pits" -> tiles = caveGenerator.generateOpenCave(width, height, 3);
      case "maze" ->
      ...
      case "bsp" -> tiles = complexGenerator.generateBSPDungeon(width, height, 5, 8);
      case "packed" -> tiles = complexGenerator.generatePackedDungeon(width, height, 10, 4, 7);
      default -> tiles = complexGenerator.generateSparseDungeon(width, height, 5, 5, 15);
    }
  ...
  private void generateFeatures(
      Collection<RZoneTheme.Feature> features, double ratio, int[][] tiles, String[][] terrain) {
